...
## 8c. Story and social systems

`story.ts`: **dilemmas**. When it is time for a choice (two or three times a month per controlled sim) the system posts a request; the store drains it and `LLMService.generateDilemma` has the model write ONE dilemma from a dossier of that life (job, money, home, the people around them with numbers, open promises, grudges, milestones, what they remember, decisions already made and their flags, this week's news, recent journal). The reply is structured: title, body, deadline, two to three options, and per option the narration, bounded player effects, effects on named people (resolved to real sim ids), lasting flags, and follow-ups days later with a probability. Every bundle passes the shared validator; once installed the consequences are stored on the dilemma and applied deterministically. Offline, a weighted seeded template catalog is used instead ( a friend's loan, a competing job offer, a parent in hospital, covering a shift versus a promise, a found wallet, a surprise medical bill, a coworker asking you to lie, car trouble, a blind-date setup). Each arrives as an interrupt, can be deferred and stays on the Live screen with a countdown, and falls to its default at the deadline. Choices set `dilemma:<template>:<option>` flags on the sim and schedule `story:followup` events days later. **News**: roughly weekly the city gets a headline for a few days (transit strike, heat wave, layoffs, gas spike, flu wave, water main break, title run, road works, festival weekend); `newsEffects(state)` is read by transport (travel time, transit availability, rideshare surge, gas), health (contagion), venue hours (a closed venue) and moods, and the headline is in the model's world context.

`social.ts`: **feed** (a few posts a day from people you know, in the phone's Social app, with replies when you post), **gossip** (a strong NPC memory about you reaches one of their friends and shifts that friend's view of you; you hear only when it reaches someone you know), **milestones** (met, first real talk, first text, hung out, came over, met their friends, dating, exclusive, married; recognised, not scored; shown as "History together" and told to the model), and **standing** per venue for controlled sims (visits, tips, trouble; a witnessed crime bans you for 30 days and removes the venue from travel; staff opinions from conversations move it; the venue sheet and the model see one line).
